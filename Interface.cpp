...
void processChoice(CourseList &courseList)
{
	...
	{
		cerr << "\nError! The database is empty." << endl
			 << endl;
	}
}
...
void unitsOfPrerequisites(const CourseList &courseList)
{
	...
	while (!finished)
	{
		...
		else
		{
			...
			for (int prereqNum : prereqSet)
			{
				Course prereqCourse;
				if (courseList.searchCourse(prereqNum, prereqCourse))
				{
					sum += prereqCourse.getCourseUnits();
				}
			}
			...
			if (sum == 0)
			{
				...
				cout << "This means you can take this course without any qualifying classes.\n"
						"Good luck on your CS journey if you decide to take this class!\n";
			}
			else
			{
